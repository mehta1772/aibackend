...
class ServiceCreate(ServiceBase):
    deduction_amount: Optional[float] = Field(0, ge=0, description="Amount to deduct from scorecard")
...
class PaymentEntry(BaseModel):
    term: str = Field(..., pattern="^(term_1|term_2|term_3)$")
...
class BookingCreate(BookingBase):
    revenue_shares: Optional[List[RevenueShare]] = None
    payment_screenshots: Optional[List[str]] = None  # URLs from S3
...
class BookingUpdate(BaseModel):
    ...
    status: Optional[BookingStatus] = None
    revenue_shares: Optional[List[RevenueShare]] = None
    payment_screenshots: Optional[List[str]] = None
