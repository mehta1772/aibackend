...
#         # Deductions - ONLY apply if booking has Term 1 payment
#         # If BDM creates booking with term_2 or term_3 only, no service deduction
#         has_term_1_payment = (booking_data.term_1 or 0) > 0
#         if has_term_1_payment:
#             for deduction in service_deductions:
#                 creator_deduction = deduction["amount"] * (creator_percentage / 100)
#                 if creator_deduction > 0:
#                     await create_scorecard_entry(
...
#         # Create deduction entries - ONLY apply if booking has Term 1 payment
#         # If booking only has term_2 or term_3 payment, no service deduction
#         has_term_1_payment = (booking.get("term_1") or 0) > 0
#         if has_term_1_payment:
#             for deduction in service_deductions:
#                 # Creator's deduction (proportional to their share)
#                 creator_deduction = deduction["amount"] * (creator_percentage / 100)
...
async def get_all_bookings(
    ...
    bdm_name: Optional[str] = Query(None, description="Filter by BDM name"),
    company_name: Optional[str] = Query(None, description="Search company name"),
    search: Optional[str] = Query(None, description="Search by company name, booking ID, or contact number"),
    status: Optional[BookingStatus] = Query(None, description="Filter by status"),
    branch: Optional[BranchName] = Query(None, description="Filter by branch"),
    # Pagination
    page: int = Query(1, ge=1),
    ...
    # Search by company name, booking ID, or contact number
    if search:
        ...
        # If search contains digits, also search contact_no
        # Strip non-digits for normalized phone matching
        search_digits = ''.join(c for c in search_term if c.isdigit())
        if search_digits and len(search_digits) >= 3:
            # Match contact_no field - use regex on raw search term to support partial matches
...
async def search_bookings(
    ...
    """Quick search across multiple fields including contact number"""
    bookings_collection = get_collection("bookings")
    ...
    # Add contact_no search if query contains digits
    q_digits = ''.join(c for c in q if c.isdigit())
    if q_digits and len(q_digits) >= 3:
        search_conditions.append({"contact_no": {"$regex": q_digits, "$options": "i"}})
