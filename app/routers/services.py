...
async def get_all_services(
    ...
    async for service in cursor:
        services.append(ServiceResponse(
            ...
            name=service["name"],
            is_active=service.get("is_active", True),
            deduction_amount=service.get("deduction_amount", 0),
            created_at=service.get("created_at", datetime.utcnow())
        ))
...
async def create_service(
    ...
    new_service = {
        "name": service_data.name,
        "is_active": True,
        "deduction_amount": service_data.deduction_amount or 0,
        "created_at": datetime.utcnow(),
        "created_by": current_user["id"]
    }
    ...
    return ServiceResponse(
        ...
        name=new_service["name"],
        is_active=new_service["is_active"],
        deduction_amount=new_service["deduction_amount"],
        created_at=new_service["created_at"]
    )
...
async def update_service(
    ...
    if service_data.deduction_amount is not None:
        update_dict["deduction_amount"] = service_data.deduction_amount
    ...
    return ServiceResponse(
        ...
        name=updated["name"],
        is_active=updated.get("is_active", True),
        deduction_amount=updated.get("deduction_amount", 0),
        created_at=updated.get("created_at", datetime.utcnow())
    )
